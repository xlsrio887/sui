...
fn new_nvar(context: &mut Context, loc: Loc) -> N::Var {
    let id = context.counter_next();
    let name = format!("{}{}{}", *TEMP_PREFIX_SYMBOL, NEW_NAME_DELIM, id).into();
    sp(
        loc,
        N::Var_ {
            name,
...
impl<'env> Context<'env> {
    ...
    pub fn new_naming_temp(&mut self, loc: Loc) -> N::Var {
        new_nvar(self, loc)
    }
    ...
    pub fn make_match_binders(
        ...
        fields
            ...
            .map(|(_, field_name, field_type)| {
                (field_name, self.new_naming_temp(field_type.loc), field_type)
            })
            .collect::<Vec<_>>()
    }
