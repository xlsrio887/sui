...
use crate::keytool::read_keypair_from_file_as_base64;

use super::write_keypair_to_file_in_base64;
...
async fn test_read_write_keystore_with_flag() {
    ...
    let res = write_keypair_to_file_in_base64(&kp_secp, &fp_secp);
    assert!(res.is_ok());

    // read from file as enum KeyPair success
    let kp_secp_read = read_keypair_from_file_as_base64(fp_secp);
    ...
    let res = write_keypair_to_file_in_base64(&kp_ed, &fp_ed);
    assert!(res.is_ok());

    // read from file as enum KeyPair success
    let kp_ed_read = read_keypair_from_file_as_base64(fp_ed);
...
async fn test_sui_operations_config() {
    ...
    let kp = SuiKeyPair::decode(
        "suiprivkey1qr2x8cgu0y2egh5x4s4h9kytsxgvltv0776gul4cjtp8tfw0pglgyclfyyh",
    )
    .unwrap();
    ...
    let kp = SuiKeyPair::decode(
        "suiprivkey1qrcfd38ngfhqrvfes20rrul28ej7dswn2hy6h0wtsgkvs9expd0qqchs02t",
    )
    .unwrap();
    let contents = vec![kp.encode().unwrap()];
...
async fn test_private_keys_ed25519() -> Result<(), anyhow::Error> {
    // private key in bech32, address in hex
    ...
    // assert correctness
    for (private_key, address) in TEST_CASES {
        let mut keystore = Keystore::from(InMemKeystore::new_insecure_for_tests(0));
    ...
    for (private_key, addr) in TEST_CASES {
        let mut keystore = Keystore::from(InMemKeystore::new_insecure_for_tests(0));
